...
hogares <- data.frame(
  Hogar = c("A", "B", "C", "D", "E"),
  Ingreso_Total = c(800, 1200, 1500, 2000, 3000),
...
### Calcula el ingreso equivalente de cada hogar utilizando θ = 1  y θ = 0.5. Completa la tabla con estos resultados. 
...
### Calcula la tasa de pobreza (proporción de hogares pobres) para ambos valores de θ. 
...
- Con $\theta = 0.5$, todos los hogares tienen ingresos equivalentes superiores a 300.  
  $$
  \text{Tasa de pobreza} = 0\%
  $$

### Explica conceptualmente qué representa el parámetro θ y por qué las tasas de pobreza difieren entre ambos escenarios. 
...
### Explica qué es una curva de Lorenz y cómo se interpreta en términos de la desigualdad de ingresos. ¿Qué representaría una línea diagonal de 45 grados? 
...
### Compara las curvas de Lorenz de Nordika y Txile en términos de su nivel general de desigualdad. ¿Cuál sociedad presenta mayor desigualdad total y por qué? 
...
### Analiza los patrones de desigualdad en ambas sociedades. ¿En qué segmentos de la distribución se concentran las principales diferencias? 
...
### Explica conceptualmente qué mide esta correlación y por qué se considera una medida de movilidad social:
...
ingresos <- data.frame(
  Persona = c("A", "B", "C", "D"),
  Gen1 = c(100, 250, 500, 950),
  Gen2 = c(50, 200, 600, 1550)
)

kable(ingresos, 
      col.names = c("Familia", "Ingreso Gen 1 (mil pesos)", "Ingreso Gen 2 (mil pesos)"),
      align = "c",
...
### Usando una línea de pobreza de 300 mil pesos per cápita, calcula la tasa de pobreza en cada generación.
...
### Calcula el *índice de Gini* y el *ratio entre el 25% más rico y el 25% más pobre* para cada generación.
...
### ¿Qué nos enseñan ambas medidas sobre la desigualdad en cada generación? Discute los resultados considerando ambas métricas. ¿Por qué podrían ofrecer diagnósticos distintos sobre la desigualdad?
...
### Calcula el porcentaje de movilidad absoluta ascendente y descendente en cada generación.
...
### Calcula la elasticidad intergeneracional de ingresos (IGE), explica qué mide y discute su magnitud en perspectiva internacional, con base en las lecturas de Corak y Torche.
...
log_data <- data.frame(
  Persona = c("A", "B", "C", "D"),
  logGen1 = c(2.00, 2.40, 2.70, 2.98),
  logGen2 = c(1.70, 2.30, 2.78, 3.19)
)
...
### Caracteriza y compara ambas generaciones en términos de sus niveles y patrones de pobreza, desigualdad y movilidad.
...
### Considerando la descomposición de Datt-Ravallion, ¿cómo podría reducirse la pobreza en la Generación 2 si se descarta el crecimiento económico como opción? Explica tu respuesta. 
